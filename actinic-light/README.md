...
想要一个根据环境亮度来以不同的频率更新RGB值的led灯吗？我们给你做了一个

当程序启动时，获取环境的温度，然后根据这个温度，设定更新led更新RGB值的频率

##硬件要求

1.[RGB LED Module(model:KY-016)](https://rap.ruff.io/raps/60)

2.[Light Intensity Sensor(model:GY-30)](https://rap.ruff.io/raps/146)

3.[Ruff开发板](https://ruff.io/zh-cn/)
...
这里的 led 就是我们在应用中用以操作设备的 ID，该命令会提示我们输入设备型号。根据标签上的信息，LED灯模块的型号是 KY-016
然后，rap 会根据外设型号，去寻找相应的驱动。

    ? model: (led) ky-016
    ? model: ky-016
    Searching supported drivers from Rap registry...
    ? select a driver for device "led"(KY-016): ky-016@1.0.5
    Installing driver...
    Downloading package "ky-016"...
    Extracting package "ky-016" (1.0.5)...
    - ky-016@1.0.5
    Adding device "led" to application configuration...
    Adding input "led/pwm-r" (pwm) to application configuration...
    Adding input "led/pwm-g" (pwm) to application configuration...
    Adding input "led/pwm-b" (pwm) to application configuration...
    Done.

继续添加光照传感器

    rap device add light-sensor
