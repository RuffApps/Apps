...
当我们遭遇危险时，我们总会希望能发出一些求救信号。

因此我们开发了这款SOS报警器，你只要按下红色按钮开关，LED灯就会发出SOS的求救信号

##硬件要求

1.[Push Button Module(model:CK002)](https://rap.ruff.io/raps/7)

2.[RGB LED Module(model:KY-016)](https://rap.ruff.io/raps/60)
...
这里的 button 就是我们在应用中用以操作设备的 ID，该命令会提示我们输入设备型号。根据标签上的信息，大按键模块的型号是 CK002
然后，rap 会根据外设型号，去寻找相应的驱动。

    ? model: (button) CK002
    ? model: CK002
    Searching supported drivers from Rap registry...
    ? select a driver for device "button"(CK002): button-gpio@1.0.0
    Installing driver...
    Downloading package "button-gpio"...
    Extracting package "button-gpio" (1.0.0)...
    - button-gpio@1.0.0
    Adding device "button" to application configuration...
    Adding input "button/gpio" (gpio) to application configuration...
    Done.
